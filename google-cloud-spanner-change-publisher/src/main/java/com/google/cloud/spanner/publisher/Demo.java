...
public class Demo {
  ...
  static void createSpannerDemoDatabase(Configuration config, Spanner spanner) {
    Preconditions.checkState(
        Configuration.isSpannerEmulator(),
        "SPANNER_EMULATOR_HOST is not set. Demo mode is only allowed if both SPANNER_EMULATOR_HOST and PUBSUB_EMULATOR_HOST has been set.");
    Preconditions.checkState(
        Configuration.isPubsubEmulator(),
        "PUBSUB_EMULATOR_HOST is not set. Demo mode is only allowed if both SPANNER_EMULATOR_HOST and PUBSUB_EMULATOR_HOST has been set.");
    SpannerEmulatorUtil.maybeCreateInstanceAndDatabase(spanner, config.getDatabaseId());
    try {
      spanner
          .getDatabaseAdminClient()
          .updateDatabaseDdl(
              config.getDatabaseId().getInstanceId().getInstance(),
              config.getDatabaseId().getDatabase(),
              ImmutableList.of(
      ...
      if (!(spannerException.getErrorCode() == ErrorCode.FAILED_PRECONDITION
          && spannerException.getMessage().contains("Duplicate name in schema"))) {
  ...
  static void createDemoSubscriptions(Configuration config) {
    ...
    for (String tableName : new String[] {"Singers", "Albums"}) {
      ...
      String subscriptionName =
          String.format("spanner-update-%s-%s", config.getDatabaseId().getDatabase(), tableName);
      PubsubEmulatorUtil.maybeCreateSubscriptions(
          config.getPubsubProject(), topicName, subscriptionName);
    }
  }
